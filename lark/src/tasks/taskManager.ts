...
export class TaskManager {
  ...
  createCodexTask(
    prompt: string,
    cwd: string,
    larkContext?: { chatId: string; messageId: string; name?: string }
  ): BridgeTask {
    ...
    const task: BridgeTask = {
      id: `task_${nanoid(8)}`,
      name: larkContext?.name || inferTaskName(prompt),
      prompt,
      cwd: target,
      status: "queued",
      createdAt: now,
      updatedAt: now,
      larkChatId: larkContext?.chatId,
      larkMessageId: larkContext?.messageId
    };
