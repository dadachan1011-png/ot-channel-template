...
$stackPidFile = Join-Path $localDir "stack.pid"
...
$modules = @(
  @{ Name = "hub"; Script = "scripts\stop-hub.ps1" },
  @{ Name = "dingtalk"; Script = "scripts\stop-bridge.ps1" }
)

if (Test-ChannelEnv "LARK_ALLOWED_OPEN_ID") {
  $modules = @(
    @{ Name = "hub"; Script = "scripts\stop-hub.ps1" },
    @{ Name = "lark"; Script = "scripts\stop-bridge.ps1" },
    @{ Name = "dingtalk"; Script = "scripts\stop-bridge.ps1" }
  )
}
...
foreach ($module in $modules) {
  ...
  Write-Output "Stopping $($module.Name)..."
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $scriptPath -ProjectRoot $moduleRoot
}

if (Test-Path $stackPidFile) {
  $pidValue = Get-Content $stackPidFile -ErrorAction SilentlyContinue
  if ($pidValue -and [int]$pidValue -ne $PID) {
    Stop-Process -Id $pidValue -Force -ErrorAction SilentlyContinue
  }
  Remove-Item $stackPidFile -Force -ErrorAction SilentlyContinue
}

Write-Output "Channel stack stopped"
